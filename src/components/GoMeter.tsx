...
export default function GoMeter({ resorts }: GoMeterProps) {
  ...
  const avgSnow24h = resorts.reduce((sum, r) => sum + r.conditions.snowDepth.newSnow24h, 0) / resorts.length
  const avgSnow48h = resorts.reduce((sum, r) => sum + (r.conditions.snowDepth.newSnow48h || 0), 0) / resorts.length
